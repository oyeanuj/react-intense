...
export default class ReactIntense extends React.Component {
    ...
    if(newX && newY) {
      ...
      if (vertical) { // VERTICAL SCANNING
        if (newY !== lastPosition) {
          ...
          console.log("Vertical Scanning", transform);
        ...
        if (newX !== lastPosition) {
          ...
          console.log("Horizontal Scanning", transform);
  ...
  render () {
    const { loaded, transform, container } = this.state
    const { caption, src, title } = this.props

    const { height, width } = container

    //TODO: transform key below needs to be set properly.
